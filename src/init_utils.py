...
def init_motion_params_with_procrustes(
...
) -> tuple[InitMotionParams, TrackObservations]:
    ...
    errs_before = np.full((num_bases, num_frames), -1)
    errs_after = np.full((num_bases, num_frames), -1)
    ...
    # vis_se3_init_3d(server, init_rots, init_ts, means_canonical[center_idcs])
    ...
    init_params = InitMotionParams(init_rots, init_ts, motion_coefs, means_cano)
    return init_params, tracks_3d
...
def run_initial_optim(
    ...
    optimizer = torch.optim.Adam(
        [
            {"params": params.motion_coefs, "lr": 1e-2},
            {"params": params.motion_rots, "lr": 1e-2},
            {"params": params.motion_transls, "lr": 3e-2},
            {"params": params.means, "lr": 1e-3},
        ],
    )
    ...
    device = params.motion_rots.device

    # TODO(Hang Gao @ 02/27): Verify this.
    ...
    for i in pbar:
        positions = params.compute_means(ts)
        ...
        small_acc_loss = params.compute_motion_bases_smoothness_loss()
        loss += small_acc_loss * w_smooth

        small_accel_loss_tracks = (
            (2 * positions[:, 1:-1] - positions[:, :-2] - positions[:, 2:])
            .norm(dim=-1)
            .mean()
        )

        loss += small_accel_loss_tracks * w_smooth * 0.5

        z_accel_loss = params.compute_z_acc_loss(ts, w2cs)
        ...
        pbar.set_description(
            ...
            f"{small_accel_loss_tracks.item():.3f} "
            f"{z_accel_loss.item():.3f} "
        )


def init_fg_gaussians_from_tracks_3d(
    tracks_3d: TrackObservations, cano_t: int
) -> GaussianParams:
    """
    using dataclasses individual tensors so we know they're consistent
    and are always masked/filtered together
    """
    num_init_frgd_gaussians = tracks_3d.xyz.shape[0]
    ...
    u = torch.rand(num_init_frgd_gaussians, 1)
    v = torch.rand(num_init_frgd_gaussians, 1)
    w = torch.rand(num_init_frgd_gaussians, 1)
    quats = torch.cat(
        [
    ...
    opacities = torch.logit(torch.full((num_init_frgd_gaussians, 1), 0.7))
...
def init_bg_gaussians(
    ...
    bg_opacities = torch.logit(torch.full((num_init_bg_gaussians, 1), 0.7))
    gaussians = GaussianParams(
        means=bg_means,
        colors=bkdg_colors,
        scales=bkdg_scales,
